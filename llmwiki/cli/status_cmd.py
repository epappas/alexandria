"""``llmwiki status`` — operational dashboard (Phase 0 baseline)."""
...
from llmwiki.config import config_path, load_config, resolve_home, resolve_workspace
from llmwiki.core.fts_integrity import check_fts_integrity
from llmwiki.core.workspace import list_workspaces
...
def status_command(
    ...
    Shows the version, llmwiki home, daemon state (always ``not running`` in
    ...
    console.print(f"[bold]llmwiki[/bold] [cyan]{__version__}[/cyan]")
    console.print()

    if not home_exists:
        console.print("[yellow]Not initialized.[/yellow] Run [cyan]llmwiki init[/cyan].")
    ...
    table.add_row(
        "Daemon",
        "[dim]not running (start with: llmwiki daemon start — Phase 6)[/dim]",
    )
    fts_label = (
