"""``llmwiki doctor`` — pass/fail health checks with actionable remediation."""
...
from llmwiki.config import config_path, resolve_home
from llmwiki.core.fts_integrity import check_fts_integrity
from llmwiki.db.connection import connect, db_path
from llmwiki.db.migrator import Migrator, MigratorError
...
def _check_home(home: Path) -> CheckResult:
    if home.exists() and home.is_dir():
        return CheckResult("llmwiki home directory exists", True, str(home))
    return CheckResult(
        "llmwiki home directory exists",
        False,
        f"missing: {home}",
        "run: llmwiki init",
    )
...
def _check_migrations(home: Path) -> CheckResult:
    if not db_path(home).exists():
        return CheckResult("schema migrations applied", False, "no database", "run: llmwiki init")
    migrator = Migrator()
    try:
        ...
        return CheckResult(
            ...
            "investigate llmwiki/db/migrations/ for unauthorised edits",
        )


def _check_fts(home: Path) -> CheckResult:
    if not db_path(home).exists():
        return CheckResult("FTS5 integrity", False, "no database", "run: llmwiki init")
    with connect(db_path(home)) as conn:
    ...
    return CheckResult(
        ...
        "run: llmwiki reindex --fts-rebuild",
    )
