"""``llmwiki mcp`` group — serve / install / status."""
...
def serve_command(
    ...
    """
    from llmwiki.mcp.server import run_stdio

    # Log to stderr (stdout is the MCP protocol channel)
    stderr_console = Console(stderr=True)
    stderr_console.print(
        f"[dim]llmwiki MCP server starting on stdio"
...
def install_command(
    ...
    """Register llmwiki as an MCP server in a client's config file.

    Writes a marker-tagged entry so ``llmwiki mcp uninstall`` can remove it
...
def status_command() -> None:
    ...
    if not configs:
        console.print("[yellow]No llmwiki MCP registrations detected.[/yellow]")
        console.print("[dim]Run: llmwiki mcp install claude-code[/dim]")
        return
...
def _install_claude_code(workspace: str | None) -> None:
    ...
    if workspace:
        ...
        if "mcpServers" not in config:
            config["mcpServers"] = {}
        config["mcpServers"]["llmwiki"] = entry
        _write_json(config_path, config)
        console.print(
            f"[green]Installed[/green] llmwiki MCP server (pinned: {workspace}) "
            f"in [bold]{config_path}[/bold]"
        )
    else:
        # Open mode → user-level config
        # Claude Code reads from multiple locations; use the CLI approach
        console.print("[dim]Running: claude mcp add llmwiki ...[/dim]")
        cmd = ["claude", "mcp", "add", "llmwiki", "--", llmwiki_bin] + args
        try:
            subprocess.run(cmd, check=True, capture_output=True, text=True)
            console.print("[green]Installed[/green] llmwiki MCP server via `claude mcp add`.")
            ...
            _write_json(config_path, config)
            console.print(
                f"[green]Installed[/green] llmwiki MCP server (open mode) "
                f"in [bold]{config_path}[/bold]"
            )
...
def _install_claude_desktop(workspace: str | None) -> None:
    ...
    _write_json(config_path, config)
    console.print(
        f"[green]Installed[/green] llmwiki MCP server in "
        f"[bold]{config_path}[/bold]"
    )
    console.print("[dim]Restart Claude Desktop to pick up the change.[/dim]")


def _find_llmwiki_bin() -> str:
    """Find the llmwiki binary path."""
    import shutil
...
def _detect_registrations() -> list[tuple[str, str, dict]]:
    """Detect installed llmwiki MCP registrations across known client configs."""
...
def _check_config(
    ...
    if "llmwiki" in servers:
        entry = servers["llmwiki"]
        if isinstance(entry, dict) and entry.get("_llmwiki_managed"):
