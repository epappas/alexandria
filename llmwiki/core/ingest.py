"""``llmwiki ingest`` — the core ingest pipeline.
...
from llmwiki.core.cascade import stage_new_page
from llmwiki.core.citations import extract_footnotes
    ...
    *,
    ...
    if db_path(home).exists():
        with connect(db_path(home)) as conn:
            from llmwiki.core.runs import insert_run_row
    ...
    if verdict.verdict in ("commit",):
        ...
        if db_path(home).exists():
            with connect(db_path(home)) as conn:
                from llmwiki.core.runs import update_run_row
                conn.execute("BEGIN IMMEDIATE")
                try:
                    update_run_row(
                        conn, run.run_id,
