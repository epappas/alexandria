...
plus a tar archive of the rest of the llmwiki home — so users have a safe
...
from llmwiki.db.connection import db_path
...
def default_backup_filename() -> str:
    ...
    return f"llmwiki-backup-{ts}.tar.gz"


def create_backup(home: Path, output: Path | None = None) -> BackupReport:
    """Create a backup tar.gz of the llmwiki home.
    ...
    """
    if not home.exists():
        raise BackupError(f"llmwiki home does not exist: {home}")
