...
`llmwiki` is a **first-class MCP server**. The guardian agent runs inside any MCP-capable client (Claude Code, Claude Desktop, Claude.ai, Cursor, Codex CLI, Windsurf, Zed, Continue, any future client) and uses `llmwiki`'s tools to read, search, and write wiki content. No custom client, no lock-in.
...
|---|---|---|---|
| **stdio** | Local agents (Claude Code, Cursor, Codex CLI, Windsurf, Claude Desktop) | The client launches `llmwiki` as a subprocess | No |
...
```bash
llmwiki mcp serve                       # open mode — all workspaces available
llmwiki mcp serve --workspace <slug>    # pinned mode — locked to one workspace
```
...
- **No authentication needed** — trust is at the OS-user level. Whoever can launch `llmwiki` as this user already has access to `~/.llmwiki/`.
...
- Optional bearer-token auth (stored in `~/.llmwiki/config.toml`) for clients that want defense-in-depth. Off by default on loopback.
...
```bash
llmwiki mcp serve --workspace customer-acme
```
...
llmwiki is single-user, but a single user routinely runs multiple MCP clients at once (Claude Code in one terminal, Claude Desktop in the menu bar, Cursor in a project repo). All three may bind the same workspace and try to invoke `write` simultaneously. Three layers of serialization make this safe:
...
2. **Per-workspace file lock.** Before any staged-run commit (the moment a `runs/<run_id>/staged/` directory is moved into the live `wiki/`), the writer acquires `~/.llmwiki/workspaces/<slug>/.lock` via `fcntl` advisory locking. The lock is held only during the move + git-commit + SQLite-state-flush — typically under 100 ms. Other writers wait up to 30 seconds.
...
The agent-as-retriever charter (`research/reference/12_agentic_retrieval.md`) assumes the connected MCP client is running a model with **≥ 128K context** and **reliable tool-use discipline**. Models that do not meet this bar will navigate llmwiki's tool surface poorly — they will burn turns, miss cascade opportunities, and skip the `overview` shortcut.
...
- Anything not explicitly tested via `llmwiki eval floor --preset <preset>`.
...
```bash
claude mcp add llmwiki -- llmwiki mcp serve
```
...
```bash
codex mcp add llmwiki -- llmwiki mcp serve
```
...
llmwiki daemon start
```
Then add `http://localhost:<port>/mcp/<workspace>` in the web connector UI. Daemon must be running for the session to work. For remote use across a LAN, bind explicitly (`llmwiki daemon start --bind 0.0.0.0 --token <generated>`) and pass the token via header.

## Bootstrapping — `llmwiki mcp install`

One-shot installers for the common clients:

```bash
llmwiki mcp install claude-code                              # open mode, user-level
llmwiki mcp install claude-code --workspace customer-acme    # pinned, writes a project .mcp.json in cwd
llmwiki mcp install claude-desktop
llmwiki mcp install cursor
llmwiki mcp install codex
llmwiki mcp install windsurf
```
...
2. **Loopback + token** — optional. Generate with `llmwiki daemon auth token`. Clients pass `Authorization: Bearer <token>`. Protects against accidentally-running-as-root or a hostile local user scenario.
3. **Non-loopback + token** — requires both `--bind` and `--token`. Daemon refuses to start on a public interface without a token.

Tokens are 32-byte random strings stored in `~/.llmwiki/secrets/daemon.token.enc`. Revoke with `llmwiki daemon auth revoke`.

## Health and observability

- `llmwiki mcp status` — lists the running stdio sessions (by parent PID) and the daemon status.
- Each tool call is logged to `~/.llmwiki/logs/mcp-<date>.jsonl` with `{ts, workspace, tool, args_hash, latency_ms, result}`. Logs are local-only, rotated weekly.
...
- No CLI passthrough. The agent cannot run arbitrary `llmwiki` commands.
...
- In open mode: no access to paths outside `~/.llmwiki/workspaces/<any-valid-slug>/`. `config.toml`, `secrets/`, `.trash/`, `templates/`, and `state.db` are not reachable.
- Never exposes raw credentials, even if the agent asks `sources` for adapter details.

## Packaging check — what ships in `pip install llmwiki`

After `pip install llmwiki`, the following must work out of the box:

1. `llmwiki --version`
2. `llmwiki init`
3. `llmwiki mcp serve` — runs on stdio, speaks MCP protocol v1, exposes every workspace.
4. `llmwiki mcp serve --workspace global` — same, pinned to the `global` workspace.
5. `llmwiki mcp install claude-code` — writes the client config.
