...
## The case for rejecting RAG as llmwiki's retrieval model
...
## The Claude Code playbook, literally renamed for llmwiki
...
## The five-primitive tool surface llmwiki must expose
...
Plus two llmwiki-specific primitives that encode provenance:
...
The agent-as-retriever bet assumes the **caller** running the agent loop is a frontier model. If a user points a weaker model at llmwiki via MCP, the pattern produces measurably worse results than a vector store would on the same workspace. This is not a flaw in the architecture — it is a **runtime capability dependency** that the user must satisfy.

`architecture/14_evaluation_scaffold.md`'s capability floor test (`llmwiki eval floor --preset <preset>`) measures this. Users running below the floor see a warning at daemon startup. The architecture does not stop working below the floor; it degrades silently, which the warning makes visible.

## Why this is not "we just forgot to build a vector store"

A vector store is *extra infrastructure* that exists to paper over the fact that the model cannot browse. The model **can** browse. At the scale llmwiki targets — workspaces of tens to a few thousand pages — the agent can reach every relevant page through navigation primitives in a bounded number of tool calls, and the reads it does make are the reads it chose.
...
## What this means for llmwiki specifically
...
5. **Subagent re-entrancy is a design requirement.** The MCP server must handle concurrent calls cleanly so that a client (Claude Code, Claude.ai) can spawn multiple subagents that all hit llmwiki at once.
...
7. **Reframe the architecture's RAG-adjacent language.** Our reference doc `09_graph_rag_literature.md` calls llmwiki "personal-scale Graph RAG." That concedes too much. The correct framing is **agentic search over a compiled wiki**, with the Graph RAG literature as a contrasting approach for larger scales.
