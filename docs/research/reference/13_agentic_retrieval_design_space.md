...
| Archetype | What it is | alexandria's stance |
|---|---|---|
...
**Conclusion:** every Tier 1 archetype assumes a *static artifact* the agent queries at runtime. alexandria's indexing archetype is "the wiki itself." The compiled markdown pages + footnote citations + `wiki_claim_provenance` table *are* the index. They are human-readable, navigable by tools, and updated through the guardian's ingest/lint passes — not through a separate pipeline.

### Tier 2 — Execution paradigms (who drives retrieval)

| Paradigm | What it is | alexandria's stance |
|---|---|---|
| **Agentic Search** (Anthropic / Claude Code) | Model drives search as a reasoning loop. No index. | **Adopted.** This is the alexandria model. `guide` → `search`/`grep`/`list` → `read` → `follow` → synthesize. |
| **Multi-Agent Orchestrator-Worker** (Anthropic Research) | Lead + parallel subagents + citation validator. External memory handoffs. | **Partially adopted.** The *client* (Claude Code, Claude.ai) owns orchestration; alexandria exposes re-entrant MCP tools so subagents can work in parallel against the same workspace. Our `wiki_log_entries` + `wiki/log.md` are the external memory the gist's pattern requires. |
...
The gist lists adaptive routing as *"considered mandatory for production cost control in 2026."* The warning targets pipeline-based systems where you must commit to a retrieval strategy before generation. alexandria is not such a system.
...
For alexandria this changes the framing from "we are a knowledge base with an MCP server" to "**we are one of several retrieval tools the agent composes**". Concretely:

1. The agent may call alexandria's `search` for a concept in the user's personal wiki, then call a separate web search tool, then come back and `read` a follow-up raw source. That's normal composition — we do not own the session.
2. alexandria's MCP server must play cleanly alongside other MCP servers registered in the same client. No state assumptions, no expectation that we're the first tool called.
...
4. The workspace binding in `08_mcp_integration.md` already handles scoping across multiple alexandria instances. We extend the same discipline across multiple retrieval tools generally.

## Multi-agent engineering lessons (applied to alexandria)

The gist extracts five engineering lessons from Anthropic's multi-agent research post. Each maps directly to a concrete requirement on alexandria's tool surface:
