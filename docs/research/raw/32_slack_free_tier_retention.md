...
- **Fetched:** 2026-04-15
- **Purpose:** determine what historical Slack content llmwiki can actually access on a free workspace.
...
3. **llmwiki can only ingest Slack history it captures while it is running.** Day-1 install → we see everything from day 1 forward. Retroactive ingest of a multi-year project's Slack history on a free workspace is impossible.
...
- Paid workspaces are unconstrained — llmwiki can pull full history on setup.
