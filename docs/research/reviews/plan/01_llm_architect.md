...
**Phase 2b (2 weeks): MV Anthropic provider + live verification.** Minimum viable Anthropic provider with streaming, error taxonomy, retry, prompt caching from day one, budget hooks including verifier multiplier and per-run ceiling. `llmwiki ingest` end-to-end wired through live Anthropic. Tests against live API with hard ceilings. Real budget smoke tests.
