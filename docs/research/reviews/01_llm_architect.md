...
3. **Add a "synthesis run envelope" section to `11_inference_endpoint.md`** defining `{run_id, started_at, status}` in SQLite + `.llmwiki/drafts/{run_id}/*.md` on disk, with a daemon-startup recovery scan. Three paragraphs max.
