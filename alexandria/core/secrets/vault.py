...
class SecretVault:
    ...
    1. ``ALEXANDRIA_VAULT_PASSPHRASE`` environment variable
    ...
    def _get_passphrase(self) -> str:
        env = os.environ.get("ALEXANDRIA_VAULT_PASSPHRASE")
        if env:
            return env
        try:
            import keyring
            stored = keyring.get_password("alexandria", "vault_master")
            if stored:
                return stored
        except (ImportError, Exception):
            pass
        raise VaultError(
            "no vault passphrase found. Set ALEXANDRIA_VAULT_PASSPHRASE or "
            "install keyring and run: alexandria secrets init"
        )
